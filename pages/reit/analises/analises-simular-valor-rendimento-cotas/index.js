...
function AnaliseAcoesSimularValorRendimentoCotas() {
    ...
    const handleSubmit = async () => {
        setRowsList([])
        const res = await fetch(REIT_ANALISE_URL + '/simula-rendimento-por-cotas/' + valorInvestimento  + '/')        
    ...
    const handleFilter = async () => {
        setRowsList([]);
        const res = await fetch(REIT_ANALISE_URL + '/filter-simula-rendimento-por-cotas/' + valorInvestimento  + '/?orderFilter=' + selectOrdenacao + '&typeOrderFilter=' + selectTipoOrdenacao)
        const list = await res.json()
        setRowsList(list)
    }
    ...
    return (
        <Layout title="Quantitative System">
            <h1>Analise Reits - Simulação Valor Rendimento por Quantidade de Cotas e Dividendos</h1>
