...
function AnaliseReitsSimularValorInvest() {
    ...
    const handleSubmit = async () => {
        setRowsList([])
        const res = await fetch(REIT_ANALISE_URL + '/simula-valor-investido/' + valorRendimento  + '/')        
    ...
    const handleFilter = async () => {
        setRowsList([]);
        const res = await fetch(REIT_ANALISE_URL + '/filter-simula-valor-investido/' + valorRendimento  + '/?orderFilter=' + selectOrdenacao + '&typeOrderFilter=' + selectTipoOrdenacao)
        const list = await res.json()
        setRowsList(list)
    }
    ...
    return (
        <Layout title="Quantitative System">
            <h1>Analises reits - Simulação Valor Investimento - Rendimento Mensal Dividendos</h1>
...
export default AnaliseReitsSimularValorInvest;