...
import { REIT_ANALISE_URL, ATIVOS_ANALISE_URL } from '../../../constants/constants';
...
function AnalisesReits({ list }) {
    ...
    const handleDelete = async () => {  
        const response = await fetch(REIT_ANALISE_URL  + '/' + acaoSelecionada, {
            method: 'DELETE'           
        })
        const data = await response.json()  
        
        const res = await fetch(REIT_ANALISE_URL )
    ...
    const handleConfirmDeleteAll = async () => {  
        const response = await fetch(REIT_ANALISE_URL  + '/delete-all-analises', {
            method: 'DELETE'           
        })
        const data = await response.json()  
    ...
    const handleSearchPapel = async () => {
        setRowsList([]);
        const res = await fetch(REIT_ANALISE_URL + '/filter-by-sigla/' + searchPapel)
        const list = await res.json()
        setRowsList(list)
    }
    ...
    const handleFilter = async () => {
        setRowsList([]);
        setSearchPapel('')
        const res = await fetch(REIT_ANALISE_URL + '/filter?orderFilter=' + selectOrdenacao + '&typeOrderFilter=' + selectTipoOrdenacao)
        const list = await res.json()
        setRowsList(list)
    }
    ...
    function handleSelect(event, select) {
        if ( select === 'mapaDividendos'){
            router.push('/reit/analises/analises-mapa-dividendos')
        }         
        else if ( select === 'simularValorInvest'){
            router.push('/reit/analises/analises-simular-valor-invest')
        } 
        else if ( select === 'simularValorRendimentoCotas'){
            router.push('/reit/analises/analises-simular-valor-rendimento-cotas')
        } 
        else if ( select === 'calculoPorcentagemCrescimentoCotacoes'){
            router.push('/reit/analises/analises-calcula-porcentagem-crescimento-cotacoes')
        } 
