...
function MapaDividendosAnaliseReits() {
    ...
    const handleSubmit = async () => {
        const res = await fetch(REIT_ANALISE_URL + '/mapa-dividendos/' + periodoInicio + '/' + periodoFim)
    ...
    const handleAddAtivoAnalise = async (sigla) => {
        ...
        const response = await fetch(ATIVOS_ANALISE_URL + '/add-analise-ativo/reit/' + sigla, {
            method: 'POST',
            body: JSON.stringify(
                {                
        ...
        alert('Ação adicionada na lista de Ativos sendo analisados: ' + sigla) 
    }

    const handleAddAnalise = async (sigla) => {

        const response = await fetch(REIT_ANALISE_URL + '/add-reit/' + sigla, {
            method: 'POST',
            body: JSON.stringify(
                {                
        ...
        const data = await response.json()           
        alert('Ação adicionada na lista de analises: ' + sigla) 
    }
    ...
    return (
        <Layout title="Quantitative System">
            <h1>Análise reits - Mapa Dividendos</h1>
...
export default MapaDividendosAnaliseReits;