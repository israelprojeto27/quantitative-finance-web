...
import { REIT_URL } from '../../constants/constants';
import { REIT_ANALISE_URL } from '../../constants/constants';
...
function Reits({ list }) {
    ...
    const handleSearchPapel = async () => {
        setRowsList([])
        if (searchPapel !== '') {
            const res = await fetch(REIT_URL + '/info-gerais-by-sigla/' + searchPapel)
            const list = await res.json()
            setRowsList(list)
        }
        else {
            const res = await fetch(REIT_URL + '/info-gerais')
            const list = await res.json()
            setRowsList(list)
        }        
    }
    ...
    const handleFilter = async () => {
        setRowsList([]);
        const res = await fetch(REIT_URL + '/filter-info-gerais?orderFilter=' + selectOrdenacao + '&typeOrderFilter=' + selectTipoOrdenacao)
        const list = await res.json()
        setRowsList(list)
    }
    ...
    function handleSelect(event, select) {
        if ( select === 'mapaDividendos'){
            router.push('/reit/mapa-dividendos')
        }        
        else if ( select === 'simularValorInvest'){            
            router.push('/reit/simular-valor-invest')
        }    
        else if ( select === 'simularValorRendimentoCotas'){            
            router.push('/reit/simular-valor-rendimento-cotas')
        }               
        else if ( select === 'simularInvestimentoVariasReits'){            
            router.push('/reit/simular-investimento-varias-reits')
        }     
        else if ( select === 'calculoPorcentagemCrescimentoCotacoes'){            
            router.push('/reit/calcula-porcentagem-crescimento-cotacoes')
        }  
        else if ( select === 'analisesReits'){            
            router.push('/reit/analises')
        }  
                ...
                <tr>
                    ...
                    <td className={classes.cardTd}>
                        ...
                        >
                            ...
                            <MenuItem onClick={(e) => handleSelect(e, 'simularInvestimentoVariasReits')}>Simular Investimento Várias Reits</MenuItem>                            
                            <MenuItem onClick={(e) => handleSelect(e, 'calculoPorcentagemCrescimentoCotacoes')}>Calcula Porcentagem Crescimento Reits</MenuItem>
                            <MenuItem onClick={(e) => handleSelect(e, 'analisesReits')}>Análises Reits</MenuItem>
                        </Menu>
                    </td>
                </tr>
            </table>
