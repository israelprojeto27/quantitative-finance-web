...
function SimularValorInvest() {
    ...
    const handleSubmit = async () => {
        setRowsList([])
        const res = await fetch(REIT_URL + '/simula-valor-investido/' + valorRendimento  + '/')        
    ...
    const handleFilter = async () => {
        setRowsList([]);
        const res = await fetch(REIT_URL + '/filter-simula-valor-investido/' + valorRendimento  + '/?orderFilter=' + selectOrdenacao + '&typeOrderFilter=' + selectTipoOrdenacao)
        const list = await res.json()
        setRowsList(list)
    }
    ...
    const handleFilterSigla = async () => {
        setRowsList([]);
        const res = await fetch(REIT_URL + '/simula-valor-investido-by-sigla/' + valorRendimento  + '/' + searchPapel)
        const list = await res.json()
        setRowsList(list)
    }
    ...
    return (
        <Layout title="Quantitative System">
            <h1>Simulação Valor Investimento - Rendimento Mensal Dividendos - reits</h1>
