...
function AnaliseBdrsSimularValorRendimentoCotas() {
    ...
    const handleSubmit = async () => {
        setRowsList([])
        const res = await fetch(BDR_ANALISE_URL + '/simula-rendimento-por-cotas/' + valorInvestimento  + '/')        
    ...
    const handleFilter = async () => {
        setRowsList([]);
        const res = await fetch(BDR_ANALISE_URL + '/filter-simula-rendimento-por-cotas/' + valorInvestimento  + '/?orderFilter=' + selectOrdenacao + '&typeOrderFilter=' + selectTipoOrdenacao)
        const list = await res.json()
        setRowsList(list)
    }
    ...
    return (
        <Layout title="Quantitative System">
            <h1>Analise BDRs - Simulação Valor Rendimento por Quantidade de Cotas e Dividendos</h1>
...
export default AnaliseBdrsSimularValorRendimentoCotas;