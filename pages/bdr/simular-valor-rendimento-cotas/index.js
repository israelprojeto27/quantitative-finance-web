...
function SimularValorRendimentoCotas() {
    ...
    const handleSubmit = async () => {
        setRowsList([])
        const res = await fetch(BDR_URL + '/simula-rendimento-por-cotas/' + valorInvestimento  + '/')        
    ...
    const handleFilter = async () => {
        setRowsList([]);
        const res = await fetch(BDR_URL + '/filter-simula-rendimento-por-cotas/' + valorInvestimento  + '/?orderFilter=' + selectOrdenacao + '&typeOrderFilter=' + selectTipoOrdenacao)
        const list = await res.json()
        setRowsList(list)
    }
    ...
    const handleFilterSigla = async () => {
        setRowsList([]);
        const res = await fetch(BDR_URL + '/simula-rendimento-por-cotas-by-sigla/' + valorInvestimento  + '/' + searchPapel)
        const list = await res.json()
        setRowsList(list)
    }
    ...
    return (
        <Layout title="Quantitative System">
            <h1>Simulação Valor Rendimento por Quantidade de Cotas e Dividendos - BDRs</h1>
