...
function EditSimularInvestimentoDetail({ router: { query } }) {
    ...
    const handleSubmit = async () => {  
        const response = await fetch(BDR_SIMULACAO_INVESTIMENTO_URL + '/save-simulacao-detail-investimento', {
            method: 'POST',
            body: JSON.stringify(
                {                
                    sigla: sigla,
    ...
    return (
        <Layout>
              <h1>Edição de Simulação de Detalhe Investimento Vários BDRs</h1>
