...
function SimularInvestimentoVariosBdrs({ simulacao }) {
    ...
    const handleUpdateValorInvestimento = async () => {
        const response = await fetch(BDR_SIMULACAO_INVESTIMENTO_URL, {
            method: 'POST',
            body: JSON.stringify(
                {                
    ...
    const handleSubmitGerarSimulacao = async () => {    
        setOpenResult(false)
        const res = await fetch(BDR_SIMULACAO_INVESTIMENTO_URL + '/simula-investivemento/' + periodoInicio + '/' + periodoFim)
    ...
    function goAddSimulacaoInvestimentoDetail(){
        router.push('/bdr/simular-investimento-varios-bdrs/add-simular-investimento-detail')
    }
    ...
    const handleDelete = async () => {        
        ...
        const response = await fetch(BDR_SIMULACAO_INVESTIMENTO_URL  + '/delete-simulacao-detail-investimento/'+ siglaSelecionada, {
            method: 'DELETE'           
        })
        const data = await response.json()        
        router.push('/bdr/simular-investimento-varios-bdrs/') 
    ...
    function handleEdit(row) {    
        router.push({
            pathname: '/bdr/simular-investimento-varios-bdrs/edit-simular-investimento-detail',
    ...
    return (
        <Layout title="Quantitative System">
            <h1>Simulação Investimento Vários BDRs</h1>
...
export default SimularInvestimentoVariosBdrs;


export async function getStaticProps(context) {
    const res = await fetch(BDR_SIMULACAO_INVESTIMENTO_URL )
